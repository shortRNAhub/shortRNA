...
.aggUnambiguous <- function(o){
    ...
    l4 <- .doag(m, sources, which(sources$transcript_type=="miRNA" & sources$status=="unique"), nFUN=function(x){ .miRNAbasename(x,T)})
    w <- which(sources$transcript_type %in% tRNAtype() & sources$status=="unique")
    l5 <- list( os=sources$gene_id[w],
...
.doag <- function(m, sources, w, by="gene_id", nFUN=function(x){x}){
    os <- sources[[by]][w]
