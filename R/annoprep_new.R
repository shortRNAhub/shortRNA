...
prepareAnnotation <- function(ensdb, genome=NULL, output_dir="",
  ...
  if(is.null(resolveSplicing)){
    p <- defaultAssignRules()$priorities
  ...
  if(!is.null(extra.seqs)){
    ...
    if(is.list(extra.seqs)){
      m <- dplyr::bind_rows(lapply(extra.seqs, FUN=function(x)
        data.frame(row.names=names(x), tx_id=names(x), seq=as.character(x))
      ), .id="tx_biotype")
  ...
  gs <- genes(ensdb, filter=~!(tx_biotype %in% resolveSplicing),
              columns=c("gene_id", "gene_biotype", "symbol"))
  gs$tx_id <- gs$symbol
  gs$tx_biotype <- gs$gene_biotype
  gs$gene_id <- gs$gene_biotype <- NULL
  tx <- exonsBy(ensdb, filter=~tx_biotype %in% resolveSplicing,
  ...
  extra.gr <- lapply(names(extra.gr), FUN=function(x){
    ...
    if(is(gr, "GRangesList")){
      ...
      stopifnot("tx_id" %in% colnames(mcols(extra.gr[[x]])))
  ...
  if(is.character(genome) && length(genome)!=1)
    stop("`genome` should be the path to a fasta(.gz) file, or a ",
         "BSgenome/TwoBitFile object")
  if(is(genome, "TwoBitFile")) genome <- import(genome)
  ...
  if(is.null(extra.seqs)){
    if(!is.character(genome)){
      saveXStringSet(genome, genome.out, compress=TRUE)
    }else{
      if(is.character(genome)){
        file.copy(genome, genome.out)
        extra <- file.path(output_dir,
                           paste0("extra.fasta",ifelse(isCompressed,".gz","")))
        writeXStringSet(extra.seqs, filepath=extra, compress=isCompressed,
                        append=TRUE)
      }else{
    ...
    rm(genome)
    message("Genome including eventual extra chromosomes was saved in:\n",
            genome.out, "\nNow building the index...")
    buildindex(basename=output_dir, reference=genome.out, ...)
    return(tx)
}