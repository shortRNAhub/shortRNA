.srcsCols <- function() {
  c(
    "seq", "cigar", "read.strand", "overlap", "startInFeature",
...
assignReads <- function(sources, rules = defaultAssignRules(), tree = NULL, BP = NULL,
  ...
  if (!all(nn %in% colnames(sources))) {
    .fstop(paste(
      "`sources` should have exactly the following column names:",
      paste(nn, collapse = ", ")
    ))
  }
...
assignRead <- function(sources, rules = defaultAssignRules(), tree = NULL) {
  ...
  if (!all(nn %in% colnames(sources))) {
    .fstop(paste(
      "`sources` should have at least the following column names:",
      paste(nn, collapse = ", ")
    ), sources)
  }
  ...
  if (nloc > 1 & nloc < nrow(sources)) {
    sources <- lapply(split(sources, sources$chrPos, drop = F),
      rules = rules,
    ...
    tRF = list(
      ...
      types = list(
        ...
        "tRNA_3p_half" = function(x) {
          x$distanceToFeatureEnd %in% -1:1 & x$length >= 34 & x$length <= 50 & grepl("CCA$", x$seq)
        },
        "tRNA_5p_fragment" = function(x) {
          x$startInFeature < 5 & x$length < 30
        },
        "tRNA_3p_fragment" = function(x) {
          x$distanceToFeatureEnd < 5 & x$length < 50
        }
      )
    ),
