...
assignReads <- function(sources, rules = defaultAssignRules(), nthreads = NULL) {
  ...
  nn <- c(
    "seq", "cigar", "chrPos", "strandRead", "strandFeature", "posInFeature",
...
assignRead <- function(sources, rules = defaultAssignRules()) {
  ...
  if (all(sources$transcript_type == "miRNA")) {
    return(.disambiguate_miRNAs(sources))
  }
  if (all(sources$transcript_type %in% c("pseudo_tRNA", "tRNA"))) {
...
check_miRNA <- function(src, length = 20:23, minOverlap = 12) {
  ...
  if (!(src$length %in% length)) {
    return(FALSE)
  }
  if (src$overlap < minOverlap) {
    return(FALSE)
  }
  return(TRUE)
}
...
.disambiguate_miRNAs <- function(sources) {
  ...
  if (length(w) > 0) {
    if (length(unique(sources$gene_id[w])) == 1) {
      return(.aggSources(sources[w, , drop = F], addStatus = "unique"))
    }
    sources <- sources[w, , drop = F]
  }
  sources$gene_id <- nn
...
.validateFeatureType <- function(src, rules) {
  if (src[["transcript_type"]] %in% names(rules$typeValidation)) {
    return(rules$typeValidation[[src[["transcript_type"]]]](src))
  }
  return(TRUE)
}

.nmax <- function(x) {
  ifelse(all(is.na(x)), NA_integer_, max(x, na.rm = T))
}
...
.tRNAnt <- function(gnames) {
  ...
  x <- lapply(x, FUN = function(x) {
    x[[2]] <- gsub("SeC(e)", "SelCys", x[[2]], fixed = T)
    x
  })
  x2 <- sapply(x, FUN = function(x) {
    if (length(x) > 2) x[[3]] <- gsub("R", "A", x[[3]], fixed = T)
    paste(x, collapse = "-")
  })
  ...
  x2 <- sapply(x, FUN = function(x) {
    if (length(x) > 2) x[[3]] <- gsub("R", "G", x[[3]], fixed = T)
    paste(x, collapse = "-")
  })
  if (length(unique(x2)) == 1) {
    return(x2)
  ...
  x2 <- sapply(x, FUN = function(x) {
    if (length(x) > 2) x[[3]] <- gsub("Y", "C", x[[3]], fixed = T)
    paste(x, collapse = "-")
  })
  if (length(unique(x2)) == 1) {
    return(x2)
  ...
  x2 <- sapply(x, FUN = function(x) {
    if (length(x) > 2) x[[3]] <- gsub("Y", "T", x[[3]], fixed = T)
    paste(x, collapse = "-")
  })
  if (length(unique(x2)) == 1) {
    return(x2)
...
checkPiRNA <- function(seqs, type = "primary", allowRevComp = FALSE, size = 26:32) {
  ...
  if (allowRevComp) {
    ...
    if (type == "primary" | type == "any") {
      pp <- apply(cbind(seqs, revcomp), 1, FUN = function(x) {
        any(sapply(strsplit(x, ""), FUN = function(x) {
          x[[1]]
        }) == "T")
      })
    }
    if (type == "secondary" | type == "any") {
      sp <- apply(cbind(seqs, revcomp), 1, FUN = function(x) {
        any(sapply(strsplit(x, ""), FUN = function(x) {
          x[[10]]
        }) == "A")
      })
    ...
    if (type == "primary" | type == "any") {
      pp <- sapply(strsplit(seqs, ""), FUN = function(x) {
        x[[1]] == "T"
      })
    }
    if (type == "secondary" | type == "any") {
      sp <- sapply(strsplit(seqs, ""), FUN = function(x) {
        x[[10]] == "A"
      })
    }
  }
...
defaultAssignRules <- function() {
  return(list(
    ...
    typeValidation = list(
      primary_piRNA = function(x) {
        checkPiRNA(x$seq, type = "primary")
      },
      secondary_piRNA = function(x) {
        checkPiRNA(x$seq, type = "secondary")
      },
      miRNA = function(x) {
        check_miRNA(x, length = 20:23, minOverlap = 12)
      }
    ),
    tRF = list(
      gtRNAdb = "prioritize", # either 'require', 'prioritize', or 'any'
      types = list(
        "tRNA_5p_half" = function(x) {
          x$posInFeature %in% -1:1 & x$length %in% 30:34
        },
        # "tRNA_3p_half"=function(x){ x$posInFeature>=(x$feature_length-x$length-4) & x$length >= 38 & x$length <= 50 },
        "tRNA_3p_half" = function(x) {
          x$posInFeature >= 35 & x$length >= 38 & x$length <= 50 & grepl("CCA$", x$seq)
        },
        "tRNA_5p_fragment" = function(x) {
          x$posInFeature < 5 & x$length < 30
        },
...
tRFtype <- function(srcs, rules = defaultAssignRules()) {
  if (rules$tRF$gtRNAdb == "require") {
    if (length(unique(srcs$transcript_type)) > 1) {
      return("tRNA;pseudo_tRNA")
    }
  }
  ...
  if (length(a) == 0) {
    return("tRNA_internal_fragment")
  }
  if (length(a) == 1) {
    return(a)
  }
  if (all(grepl("5p", a))) {
    return("tRNA_5p_fragment")
  }
  if (all(grepl("3p", a))) {
    return("tRNA_3p_fragment")
  }
