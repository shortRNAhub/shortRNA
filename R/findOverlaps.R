...
overlapWithTx2 <- function(bamFile, annotation, ignoreStrand = TRUE, nbthreads = NULL) {
  ...
  if (any(toResolve)) {
    ...
    posInFeature[w, ] <- t(bpmapply(
      FUN = .posInFeature, read = OV@first[w, ], feature = OV@second[w, ], Rd = Rdiff[w], oi = OVinter[w],
      BPPARAM = bp
    ))
    w <- intersect(w, negative_features)
    posInFeature[w, 1:2] <- posInFeature[w, 2:1]
  ...
  res <- data.table(
    seq = as.character(OV@first@elementMetadata$seq), cigar = OV@first@elementMetadata$cigar, chr = .ext_rlelist(
      OV@first,
      seqnames
    ), read.start = sapply(start(OV@first), FUN = min), read.end = sapply(end(OV@first), FUN = max), read.strand = .ext_rlelist(
      OV@first,
      strand
    ), overlap = sapply(width(OVinter), FUN = sum), startInFeature = posInFeature[, 1], distanceToFeatureEnd = posInFeature[
      ,
      2
    ], transcript_id = mcols(OV@second)$transcript_id, transcript_type = factor(mcols(OV@second)$transcript_type), transcript.strand = strands,
    gene_id = mcols(OV@second)$gene_id, transcript.length = sapply(width(OV@second), FUN = sum)
  )
  rm(OV, OVinter)

  # add alignments that did not overlap anything
  nonOV <- suppressWarnings(subsetByOverlaps(bam, annotation, invert = TRUE, ignore.strand = ignoreStrand))

  res2 <- data.table(seq = as.character(nonOV@elementMetadata$seq), cigar = nonOV@elementMetadata$cigar, chr = .ext_rlelist(
    nonOV,
    seqnames
  ), read.start = sapply(start(nonOV), FUN = min), read.end = sapply(end(nonOV), FUN = max), read.strand = .ext_rlelist(
    nonOV,
    strand
  ))
...
.posInFeature <- function(read, feature, Rd, oi) {
  c(.startInFeature(read, feature, Rd, oi), .endInFeature(read, feature, Rd, oi))
}
...
.startInFeature <- function(read, feature, Rd, oi) {
  ...
  return(as.integer(sum(width(feature)[which(end(feature) < rs)]) + rs - start(feature)[w]))
}
...
.endInFeature <- function(read, feature, Rd, oi) {
  ...
  return(as.integer(-1 * (sum(width(feature)[which(start(feature) > rs)]) + end(feature)[w] - rs)))
}
