...
mergeAtomicLists <- function(x,y){
  ...
  if(is(x,"FactorList") && is(y,"FactorList")){
    ...
    return(splitAsList(factor(c(fx,fy),levels=seq_along(lvls),lvls),
                       c(rep(n,lengths(x)), rep(n,lengths(y)))))
  }else{
    if(is(x,"FactorList")) x <- CharacterList(x)
    if(is(y,"FactorList")) y <- CharacterList(y)
  }
  splitAsList(c(unlist(x,use.names=FALSE), unlist(y,use.names=FALSE)),
              c(rep(n,lengths(x)), rep(n,lengths(y))))
}
...
fList2tree <- function(fL, addRoot=TRUE, collapseSingles=FALSE, root = "ROOT"){
  ...
  if(collapseSingles) o <- ape::collapse.singles(tree = o, root.edge = TRUE)
  o
}
