...
server <- shinyServer(function(input, output) {
  ...
  output$title2 <- renderText({o@description})
  ...
  output$seqc <- renderPlot({
    if(length(s())==0){
      ...
      if(length(s())>100){
        plot.new(); legend("topleft",legend="Too many matching sequences to plot...")
      }else{
  ...
  output$sizeplot <- renderPlot({ checkSizeSelection(o) })

  output$info <- renderPrint({ res$info })
