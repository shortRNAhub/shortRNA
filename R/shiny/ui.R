  ...
  mainPanel(
    tabsetPanel(
      tabPanel("Overview", fluidRow(
        column(3, selectInput("featureType", label="Feature type", choices=c(   "seq-level: all known"="seq.all","seq-level: miRNA"="seq.miRNA", "seq-level: tRNA"="seq.tRNA",
	...
	column(7, plotOutput("pca", height=500, width=600))
      ...
      tabPanel("Sequences", fluidRow(
          column(2, textInput("findFeature", label=h3("Find feature(s)"), value = ""),
      ...
      tabPanel("Differential expression", fluidRow(
        column(3, selectInput("deaTypes", label="Feature type", choices=c(      "seq-level: all known"="seq.allKnownUnique","seq-level: miRNA"="seq.miRNA","seq-level: tRNA"="seq.tRNA",
